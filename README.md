...
| `.claude/rules/` | `git-workflow` / `security` / `codex-workflow` | **更新**（プラグインが正典） |
...
- 認証: 環境変数 `GITHUB_PERSONAL_ACCESS_TOKEN` が必要（`Bearer` ヘッダに展開される）。トークンはソースに書かず環境変数で渡す（[`security.md`](.claude/rules/security.md) 参照）。

**`codex`** (v1.0.4) — OpenAI 公式。本リポジトリの `.claude/rules/codex-workflow.md` / `.codex/` と連携する。
...
| `.claude/rules/` | `git-workflow` / `security` / `codex-workflow` | 更新 |
...
ルートには設定ファイルを置かず、配布対象はすべて `plugins/my-plugin/` 配下にある。
...
└── templates/                # /setup-project が各プロジェクトに展開する雛形
    ├── CLAUDE.md / AGENTS.md / RULES.md / SOUL.md
    ├── rules/                # git-workflow / security / codex-workflow + 固有プレースホルダ
...
`plugins/my-plugin/commands/<name>.md` に作成。フロントマターと本文の書式は既存の `read-pdf.md` / `setup-project.md` を参照。
